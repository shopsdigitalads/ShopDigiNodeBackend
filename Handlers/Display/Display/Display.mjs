      ...
      for (const date of Object.keys(data)) {
        ...
        if (display_earning.length > 0) {
          ...
          await pool.query(
            ...
            [earning, fine, earning, ad_count, total_active, totalInactiveMinutes - total_active, display_id, date]
          );
        } else {
          await pool.query(
            `INSERT INTO DisplayEarning (active_time, inactive_time, earning_date, display_id, total_earning, fine, earning,ad_count) 
             VALUES (?, ?, ?, ?, ?, ?, ?,?)`,
            [data[date].active_time, inactive_time, date, display_id, earning, fine, earning,ad_count]
          );
        }
      }
